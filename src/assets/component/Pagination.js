  ...
  connectedCallback() {
    ...
    this.innerHTML = `
      <div class="flex justify-center items-center gap-2 md:gap-4 mt-16 mb-10 font-label">
        <!-- Previous Button -->
        <button 
          class="flex items-center gap-2 px-4 py-2 bg-white border-2 border-black shadow-[3px_3px_0_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed group"
          ${current === 1 ? "disabled" : ""}
        >
    ...
    for (let i = 1; i <= total; i++) {
      ...
      pages += `
        <button 
          ...
          ${
            isActive
              ? "bg-sidebar shadow-none translate-x-[2px] translate-y-[2px]"
              : "bg-white shadow-[3px_3px_0_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] hover:bg-bro"
          }"
        >
          ${i}
        </button>
